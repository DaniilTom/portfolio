...
namespace DatabaseWrapper
{
    public static class ItemTemplateRepository
    {
        ...
        public static IEnumerable<ItemDto> GetAll()
        {
            var dt = DatabaseWrapper.GetTable("select Id from 'item_template';");
            foreach (DataRow row in dt.Rows)
            {
                yield return GetById(row.GetInt("Id"));
            }
        }

        public static IEnumerable<ItemLocale> GetItemLocales()
        {
            var dt = DatabaseWrapper.GetTable($"select ItemId from '{_localeTables[GetCurrentLocale()]}';");
            foreach (DataRow row in dt.Rows)
            {
                yield return GetLocaleByItemId(row.GetInt("ItemId"));
            }
        }
        #endregion
        ...
        public static ItemDto GetById(int id)
        {
            ...
            try
            {
                ...
                var item = new ItemDto
                {
                    ...
                    Class = (ItemClasses) dr.GetInt(ITEM_TEMPLATE_CLASS),
                    ...
                    InventorySlot = (InventorySlots) dr.GetInt(ITEM_TEMPLATE_INVENTORYSLOT),
                    ItemLevel = dr.GetInt(ITEM_TEMPLATE_ITEMLEVEL),
                    ...
                    Delay = dr.GetInt(ITEM_TEMPLATE_DELAY),
                    ...
                    ItemSet = dr.GetInt(ITEM_TEMPLATE_ITEMSET),
        ...
        /// <summary>
        /// Метод получения объекта локали
        /// </summary>
        /// <param name="id">ID предмета</param>
        /// <returns></returns>
        public static ItemLocale GetLocaleByItemId(int itemId)
        {
            try
            {
                ItemLocale iLocale = new ItemLocale();

                var dtl = DatabaseWrapper.GetTable(
                    $"select * from '{_localeTables[GetCurrentLocale()]}' where ItemId={itemId} limit 1;");
                var drl = dtl.Rows[0];
                iLocale.Id = drl.GetInt(ITEM_TEMPLATE_LOCALE_ID);
                iLocale.ItemId = drl.GetInt(ITEM_TEMPLATE_LOCALE_ITEMID);
                iLocale.Title = drl.GetString(ITEM_TEMPLATE_LOCALE_TITLE);
                iLocale.FlavorText = drl.GetString(ITEM_TEMPLATE_LOCALE_FLAVORTEXT);
